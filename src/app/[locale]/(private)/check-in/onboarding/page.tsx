  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      ...
      if (status === "unauthenticated") {
        router.push(
          `/auth/signin?callbackUrl=/check-in/onboarding?eventId=${
            eventIdFromUrl || ""
          }`
        );
        return;
      ...
      if (session?.user) {
        ...
        if (userData) {
          ...
          setFormData((prev) => ({
            ...
            isZionMember: userData.churchLink
              ? "true"
              : userData.otherChurch
              ? "false"
              : "true",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const onboardingData = {
        ...
        churchId: churchId,
        otherChurch:
          formData.isZionMember === "false" ? formData.otherChurch : null,
        userId: session?.user?.id,
      };
      ...
      toast({
        ...
        description:
          "Ocorreu um erro ao processar seus dados. Tente novamente mais tarde.",
        variant: "destructive",
        duration: 5000,
      });
    }
            ...
            <CardContent className="space-y-6 px-6">
              ...
              <div className="space-y-3">
                ...
                <RadioGroup
                  ...
                  onValueChange={(value) =>
                    setFormData((prev) => ({ ...prev, isZionMember: value }))
                  }
                  className="flex space-x-6 pt-2"
                >
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="true" id="zion-yes" />
                    <Label htmlFor="zion-yes" className="cursor-pointer">
                      Sim
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="false" id="zion-no" />
                    <Label htmlFor="zion-no" className="cursor-pointer">
                      Não
                    </Label>
                  </div>
                </RadioGroup>
              ...
              {formData.isZionMember === "true" ? (
                <div className="space-y-3">
                  ...
                  <RadioGroup
                    ...
                    onValueChange={(value) =>
                      setFormData((prev) => ({ ...prev, isSameChurch: value }))
                    }
                    className="flex space-x-6 pt-2"
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      <Label
                        htmlFor="same-church-yes"
                        className="cursor-pointer"
                      >
                        Sim, frequento esta Zion
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <RadioGroupItem value="false" id="same-church-no" />
                      <Label
                        htmlFor="same-church-no"
                        className="cursor-pointer"
                      >
                        Frequento outra Zion
                      </Label>
                    </div>
                  </RadioGroup>
              ...
              <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                <p className="text-sm text-blue-800 font-medium mb-1">
                  ✨ Só precisará fazer isso uma vez!
                </p>
                <p className="text-xs text-blue-700">
                  Você não precisará repetir esse processo nas próximas vezes
                  que fizer check-in.
                </p>
              </div>
            </CardContent>
                ...
                {currentCheckIn && (
                  <div className="bg-gray-50 rounded-lg p-4 mb-6 w-full max-w-xs mx-auto">
                    <p className="font-medium text-gray-800">
                      {currentCheckIn.event?.name || "Check-in realizado"}
                    </p>
                    <div className="flex items-center mt-2 text-sm text-gray-600">
                      <Clock size={16} className="mr-2" />
                      <span>
                        {currentCheckIn.prayerTurn?.startTime || "--:--"} -{" "}
                        {currentCheckIn.prayerTurn?.endTime || "--:--"}
                      </span>
                    </div>
                    <div className="flex items-center mt-2 text-sm text-gray-600">
      ...
      return (
        <Card className="w-full max-w-md shadow-lg bg-white">
          ...
          <CardContent className="flex flex-col items-center justify-center py-8">
            ...
            <Button onClick={() => router.push("/check-in")} className="w-full">
              Voltar
            </Button>
          </CardContent>
        </Card>
